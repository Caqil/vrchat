...
type UserSettings struct {
	...
	UserID                string                `bson:"user_id" json:"user_id"`                                 // user identifier
	Theme                 string                `bson:"theme" json:"theme"`                                     // light, dark, auto
	Language              string                `bson:"language" json:"language"`                               // preferred language code
...
type BlockedUser struct {
	...
	BlockedUser string             `bson:"blocked_user" json:"blocked_user"` // user who was blocked
...
func (s *SettingsService) GetPublicSettings() (map[string]interface{}, error) {
	...
	publicSettings := map[string]interface{}{
		...
		"max_users":       settings.MaxUsersPerRoom,
		"chat_timeout":    settings.ChatTimeout,
		"min_age":         settings.MinimumAge,
		"maintenance":     settings.MaintenanceMode,
		"maintenance_msg": settings.MaintenanceMessage,
...
// BlockUser blocks a user for another user
func (s *SettingsService) BlockUser(userID, blockedUserID, reason string) error {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	// Check if already blocked
	exists, err := s.IsUserBlocked(userID, blockedUserID)
	if err != nil {
		return err
	}
...
// UnblockUser unblocks a user for another user
func (s *SettingsService) UnblockUser(userID, blockedUserID string) error {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	result, err := s.blockedUsersCollection.DeleteOne(ctx, bson.M{
		"user_id":      userID,
		"blocked_user": blockedUserID,
	})
	if err != nil {
	...
	if result.DeletedCount == 0 {
		return fmt.Errorf("user was not blocked")
	}
...
func (s *SettingsService) IsUserBlocked(userID, targetUserID string) (bool, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	count, err := s.blockedUsersCollection.CountDocuments(ctx, bson.M{
		"user_id":      userID,
...
func (s *SettingsService) GetBlockedUsers(userID string) ([]BlockedUser, error) {
	...
	cursor, err := s.blockedUsersCollection.Find(ctx, bson.M{"user_id": userID})
	if err != nil {
...
// ExportUserSettings exports all user settings to a map
func (s *SettingsService) ExportUserSettings(userID string) (map[string]interface{}, error) {
	...
	logger.LogUserAction(userID, "settings_exported", nil)

	return exportData, nil
}

// ImportUserSettings imports user settings from a map
func (s *SettingsService) ImportUserSettings(userID string, importData map[string]interface{}, overwrite bool) error {
	...
	_, err = session.WithTransaction(ctx, func(sc mongo.SessionContext) (interface{}, error) {
		...
		if settingsData, ok := importData["user_settings"].(map[string]interface{}); ok {
			if !overwrite {
...
// ================================
// Public Methods for Default Settings
// ================================

// GetDefaultPrivacySettings returns default privacy settings (public method)
func (s *SettingsService) GetDefaultPrivacySettings() *PrivacySettings {
	return s.getDefaultPrivacySettings()
}

// GetDefaultChatPreferences returns default chat preferences (public method)
func (s *SettingsService) GetDefaultChatPreferences() *ChatPreferences {
	return s.getDefaultChatPreferences()
}

// GetDefaultNotificationSettings returns default notification settings (public method)
func (s *SettingsService) GetDefaultNotificationSettings() *NotificationSettings {
	return s.getDefaultNotificationSettings()
}

// GetDefaultAccessibilitySettings returns default accessibility settings (public method)
func (s *SettingsService) GetDefaultAccessibilitySettings() *AccessibilitySettings {
	return s.getDefaultAccessibilitySettings()
}
...
func (s *SettingsService) mapToChatPreferences(data map[string]interface{}) *ChatPreferences {
	...
	if val, ok := data["max_chat_duration"].(int); ok {
		prefs.MaxChatDuration = val
	}
...
// GetModerationSettings retrieves moderation-specific settings
func (s *SettingsService) GetModerationSettings() (map[string]interface{}, error) {
	...
	return map[string]interface{}{
		...
		"auto_ban_threshold":      3, // Default value if not set
		"report_threshold":        5, // Default value if not set
	}, nil
}

// UpdateModerationSettings updates moderation-specific settings
func (s *SettingsService) UpdateModerationSettings(updateData map[string]interface{}) error {
	// Filter only moderation settings fields
	allowedFields := map[string]bool{
		"enable_moderation":       true,
		"enable_profanity_filter": true,
		"banned_words":            true,
		"banned_countries":        true,
		"auto_ban_threshold":      true,
